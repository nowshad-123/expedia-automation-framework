...
public class TravellersComponent extends BasePage {
    ...
    private static final Logger logger =
        LogManager.getLogger(TravellersComponent.class);
    ...
    private final By travellersInput =
        By.xpath("//div[@data-cy = 'flightTraveller']");

    // Adults selector boxes
    // Returns 10 li elements: 1,2,3...9,>9
    private final By adultBoxes = By.xpath(
        "//ul[contains(@class,'guestCounter')]" +
        "//li[contains(@data-cy,'adults')]"
    );

    // Children selector boxes
    // Returns 8 li elements: 0,1,2...6,>6
    private final By childBoxes = By.xpath(
        "//ul[contains(@class,'guestCounter')]" +
        "//li[contains(@data-cy,'children')]"
    );

    // Infants selector boxes
    // Returns 8 li elements: 0,1,2...6,>6
    private final By infantBoxes = By.xpath(
        "//ul[contains(@class,'guestCounter')]" +
        "//li[contains(@data-cy,'infants')]"
    );

    // Travel class selector boxes
    // Returns 4 li elements: Economy, Premium Economy,
    // Business, First
    private final By travelClassBoxes = By.xpath(
        "//ul[contains(@class,'guestCounter')]" +
        "//li[contains(@data-cy,'travelClass')]"
    );
    ...
    /**
     * Opens travellers panel, sets all counts,
     * selects travel class, and clicks APPLY.
     *
    ...
    public void setTravellers(
        ...
        logger.info("Setting travellers — Adults:{} " +
            "Children:{} Infants:{} Class:{}",
    ...
    /**
     * Convenience method — 1 adult, Economy class.
     * Use for basic search scenarios.
     */
    ...
    /**
     * Selects adult count by clicking the
     * corresponding li box.
     * Adults list: index 0=1adult, 1=2adults...
     * (list starts from 1, not 0)
     */
    private void selectAdults(int count) {
        ...
        if (count < 1 || count > 9) {
            throw new IllegalArgumentException(
                "Adults must be between 1 and 9. Got: "
                + count);
        }
        // Position = count - 1 (0-indexed)
        // Adult 1 = position[0], Adult 2 = position[1]
        clickBoxByPosition(adultBoxes, count - 1,
            "adult", count);
    }

    /**
     * Selects children count.
     * Children list: index 0=0children, 1=1child...
     */
    private void selectChildren(int count) {
        logger.info("Selecting {} child(ren)", count);
        if (count < 0 || count > 6) {
            throw new IllegalArgumentException(
                "Children must be between 0 and 6. Got: "
                + count);
        }
        // Children boxes show 0,1,2...6,>6
        // Index = count (since list starts at 0)
        clickBoxByPosition(childBoxes, count,
            "children", count);
    }

    /**
     * Selects infants count.
     * Infants list: index 0=0infants, 1=1infant...
     */
    private void selectInfants(int count) {
        logger.info("Selecting {} infant(s)", count);
        if (count < 0 || count > 6) {
            throw new IllegalArgumentException(
                "Infants must be between 0 and 6. Got: "
                + count);
        }
        clickBoxByPosition(infantBoxes, count,
            "infants", count);
    }
    ...
    private void selectTravelClass(TravelClass travelClass) {
        logger.info("Selecting travel class: {}",
            travelClass.getDisplayText());

        // Find the li that contains the class text
        By classLocator = By.xpath(
            "//ul[contains(@class,'guestCounter')]" +
            "//li[contains(@data-cy,'travelClass')" +
            " and contains(.,'" +
            travelClass.getDisplayText() + "')]"
        ...
        try {
            click(classLocator);
            logger.info("Travel class selected: {}",
                travelClass.getDisplayText());
        } catch (Exception e) {
            logger.warn("Could not select travel class: {}. " +
                "Using default.", travelClass.getDisplayText());
        }
    }

    /**
     * Clicks a box at a specific index position
     * from a list of li elements.
     *
     ...
     */
    private void clickBoxByPosition(
            By listLocator,
            int index,
            String label,
            int value) {
        ...
        try {
            ...
            logger.error("Failed to select {} = {} " +
                "at index {}: {}",
                label, value, index, e.getMessage());
    ...
    /**
     * Extracts XPath string from a By.xpath locator.
     * Used to build nth-element selectors.
     */
