...
public class BasePage {
	...
	protected WebElement waitForElement(By locator, WaitStrategy strategy) {
		...
		case VISIBLE_ALL -> throw new UnsupportedOperationException("Use waitForElements() for VISIBLE_ALL strategy");
		};
	}
	...
	protected void click(By locator) {
		...
		while (attempts < 3) {
			try {
				...
				attempts++;
				logger.warn("Stale element on click attempt {}. " + "Retrying: {}", attempts, locator);
			} catch (ElementClickInterceptedException e) {
				logger.warn("Click intercepted — trying JS click: {}", locator);
				jsClick(locator);
				return;
			}
		}
		throw new RuntimeException("Click failed after 3 attempts due to stale element: " + locator);
	}
	...
	protected String getAttribute(By locator, String attribute) {
		String value = waitForElement(locator, WaitStrategy.PRESENCE).getAttribute(attribute);
	...
	void waitForPageLoad() {
		...
		while (attempts < 3) {
			try {
				...
				attempts++;
				logger.warn("Page load timeout attempt {}/3", attempts);
				if (attempts == 3) {
					logger.error("Page load failed after " + "3 attempts — continuing anyway");
				}
			}
		}
	}
	...
	public void waitForDomUpdate() {
		...
		// Wait for jQuery AJAX requests if present
		try {
			wait.until(driver -> (Boolean) ((JavascriptExecutor) driver)
					.executeScript("return window.jQuery != undefined " + "&& jQuery.active === 0"));
		} catch (Exception ignored) {
			// Ignore if site doesn't use jQuery
		}

		// Small DOM stabilization delay
		try {
			Thread.sleep(500);
