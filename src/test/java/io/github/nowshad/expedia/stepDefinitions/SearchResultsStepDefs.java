...
public class SearchResultsStepDefs {
	...
	private static final Logger logger = LogManager.getLogger(SearchResultsStepDefs.class);
	...
	public void verifyResultsPageLoaded() {
		...
		Assert.assertTrue(resultsPage.isResultsPageLoaded(),
				"Results page not loaded. URL: " + resultsPage.getSearchUrl());
	}
	...
	public void urlShouldContainOriginAndDest(String origin, String dest) {
	...
	public void urlShouldContainTripType(String tripType) {
		...
		logger.info("Checking trip type {} in URL: {}", tripType, url);
		Assert.assertTrue(url.contains(tripType), "Trip type '" + tripType + "' not found in URL: " + url);
	}
	...
	public void resultsShouldHaveAtLeastNFlights(int minCount) {
		
		int actual = resultsPage.getResultCount();
		logger.info("Expected min: {} | Actual: {}", minCount, actual);
		Assert.assertTrue(actual >= minCount, "Expected at least " + minCount + " flights. Found: " + actual);
	}
	...
	public void allFlightsShouldHaveValidPrices() {
		Assert.assertTrue(resultsPage.allResultsHaveValidPrices(), "One or more flights have invalid prices");
	}

	@And("all flights should have airline names")
	public void allFlightsShouldHaveAirlineNames() {
		Assert.assertTrue(resultsPage.allResultsHaveAirlineNames(), "One or more flights missing airline name");
	}
	...
	public void firstFlightPriceGreaterThan(int minPrice) {
		...
		Assert.assertTrue(actual > minPrice, "Expected price > " + minPrice + " | Actual: " + actual);
	}

	@And("the first flight should have an airline name")
	public void firstFlightShouldHaveAirlineName() {
		String airline = resultsPage.getFirstFlightAirline();
		logger.info("First flight airline: {}", airline);
		Assert.assertFalse(airline.isEmpty(), "First flight has no airline name");
	}

	@And("no results message should not be displayed")
	public void noResultsMessageAbsent() {
		Assert.assertTrue(resultsPage.isNoResultsMessageAbsent(), "No results message showing unexpectedly");
	}
}